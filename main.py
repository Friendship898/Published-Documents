...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Semantic PNG red highlight -> orange recolor with soft masks and brightness compensation."
    )
    parser.add_argument("--input", required=True, type=Path, help="Input directory")
    parser.add_argument("--output", required=True, type=Path, help="Output directory")
    parser.add_argument("--target-color", default="#FF8A00", help="Target hex color")
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Generate N before/after previews (default when flag given: 5)",
    )
    parser.add_argument(
        "--preview-mask",
        action="store_true",
        help="Also save semantic weight masks for processed files",
    )
...
def format_log(result: ProcessResult) -> str:
    return (
        ...
        f"magenta_pixels={result.magenta_pixels} output={result.output_path} message={result.message}"
    )


def save_preview(before_path: Path, after_path: Path, preview_path: Path) -> None:
    with Image.open(before_path).convert("RGBA") as before, Image.open(after_path).convert(
        "RGBA"
    ) as after:
...
def main() -> int:
    ...
    LOGGER.info(
        "Found %d PNG files | mode=%s recursive=%s dry_run=%s workers=%d preview=%s max_files=%d",
        len(png_files),
        args.mode,
        args.recursive,
        args.dry_run,
