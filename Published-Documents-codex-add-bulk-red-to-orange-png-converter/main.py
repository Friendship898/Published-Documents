...
def validate_input_output_dirs(input_dir: Path, output_dir: Path) -> str | None:
    ...
    try:
        ...
        return None
    return "Output directory cannot be inside the input directory."
...
def main() -> int:
    ...
    validation_error = validate_input_output_dirs(args.input, args.output)
    if validation_error is not None:
        LOGGER.error("%s input=%s output=%s", validation_error, args.input, args.output)
        return 2
    ...
    LOGGER.info(
        "Found %d PNG files | mode=%s source=%s target=%s recursive=%s dry_run=%s workers=%d preview=%s max_files=%d",
        len(png_files),
        args.mode,
        args.source_color,
        args.target_color,
